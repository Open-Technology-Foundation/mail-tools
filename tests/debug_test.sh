...
echo "Standalone:"
MAILHEADERCLEAN="From" ../build/bin/mailheaderclean "$TEST_FILE" | grep "^From:" && echo "  ERROR: From found!" || echo "  ✓ From removed"
MAILHEADERCLEAN="From" ../build/bin/mailheaderclean "$TEST_FILE" | grep "^Date:" && echo "  ✓ Date kept" || echo "  ERROR: Date missing!"
echo
